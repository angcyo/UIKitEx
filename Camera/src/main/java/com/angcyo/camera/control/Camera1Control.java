 ...
 */
package com.angcyo.camera.control;
...
import com.angcyo.camera.preview.CameraView;
...
public class Camera1Control implements ICameraControl {
     ...
     * 非扫描模式
     */
    public static final int MODEL_NOSCAN = 0;
    /*
     * 本地质量控制扫描模式
     */
    public static final int MODEL_SCAN = 1;
    ...
    private AtomicBoolean pauseScan = new AtomicBoolean(false);
    ...
    private Camera.PreviewCallback previewCallback = new Camera.PreviewCallback() {
        ...
        public void onPreviewFrame(final byte[] data, Camera camera) {
            // 扫描成功阻止打开新线程处理
            if (abortingScan.get() || pauseScan.get()) {
                return;
            }
            // 节流
    ...
    @Override
    public AtomicBoolean getPauseScan() {
        return pauseScan;
    }
    ...
    private void initCamera() {
        try {
            ...
            setPreviewCallbackImpl();
            //startPreview(false); //opPreviewSize里面已经调用了
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
